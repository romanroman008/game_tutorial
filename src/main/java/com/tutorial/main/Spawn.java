...
import com.tutorial.main.objects.enemies.*;
...
public class Spawn {
    ...
    public void tick() {
        ...
        else if(hud.getDifficulty()==1){
            if (hud.getScore()==1) {
                handler.addObject(new UnpredictableEnemy(1, 1, ID.Enemy, handler));
            }
            if (scoreKeep >= 100) {
                scoreKeep = 0;
                hud.setLevel(hud.getLevel() + 1);
